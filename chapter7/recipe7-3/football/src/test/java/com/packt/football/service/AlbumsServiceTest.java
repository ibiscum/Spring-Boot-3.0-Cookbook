...
class AlbumsServiceTest {
    ...
    public void useAllCardAvailable() {
        // ARRANGE
        User user = usersService.createUser("Test");
        albumsService.buyAlbum(user.id(), "sample album");
        albumsService.buyCards(user.id(), 2);
    ...
    void tradeAllCards() {
        ...
        User user1 = usersService.createUser("Test");
        User user2 = usersService.createUser("Test2");
        albumsService.buyAlbum(user1.id(), "sample album");
        albumsService.buyAlbum(user2.id(), "sample album");
        albumsService.buyCards(user1.id(), 800);
        albumsService.buyCards(user2.id(), 800);
    ...
    void getUserWithCardsAndAlbums() {
        ...
        Album album1 = albumsService.buyAlbum(user1.id(), "sample album");
        List<Card> cards = albumsService.buyCards(user1.id(), 5);
        albumsService.addCardToAlbum(cards.getFirst().id(), album1.id());
