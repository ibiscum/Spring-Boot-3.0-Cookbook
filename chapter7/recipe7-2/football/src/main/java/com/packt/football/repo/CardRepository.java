...
public interface CardRepository extends JpaRepository<CardEntity, Integer> {
    ...
    @Query(nativeQuery = true, value = "UPDATE cards " + //
            ...
            + //
            "WHERE cards.id = r.card_id " + "RETURNING cards.*")
    List<CardEntity> assignCardsToUserAlbums(Integer userId);

    @Modifying
    @Query(value = "UPDATE CardEntity " + " SET album = null, " + " owner= (SELECT u FROM UserEntity u WHERE u.id=?2) "
            + "WHERE id = ?1 ")
    Integer transferCard(Integer cardId, Integer userId);

    @Modifying
    @Query(nativeQuery = true, value = "UPDATE cards " + "SET owner_id=?2 "
            ...
            + "WHERE cards.id = cards_from_user1_for_user2.id " + "RETURNING cards.*")
